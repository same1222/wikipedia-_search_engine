...
# Or if in Vertex AI / Local:
# !gcloud auth application-default login

PROJECT_ID = 'assignemnt-3-gcp'

def get_bucket(bucket_name):
    return storage.Client().bucket(bucket_name)

fs = gcsfs.GCSFileSystem()  # uses Dataproc credentials

def _open(path, mode, bucket=None):
    if bucket is None:
        return open(path, mode)
    return fs.open(f"{bucket.name}/{path}", mode)
...
class InvertedIndex:
    @property
    def df(self):
        # backwards-compatible alias used by read_a_posting_list / posting_lists_iter
        return self.document_frequencey_per_term
    ...
    def __init__(self, docs={}):
        ...
        self.document_frequency_per_term = Counter()
